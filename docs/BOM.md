...
|---|---|---:|---|---:|
| LilyGO T-Beam | v2.x — ESP32, NEO-M8N GPS, **SX1278** LoRa 433 MHz variant (T-Beam also ships an SX1276 868/915 variant — must match the base station), 18650 interface | 1 | Collar core: MCU + positioning + radio | 1,800–2,800 |
...
| Service-mode sensor | Glass NO reed, same part as the buckle reed (passive, 0 µA in sleep — ideal) · or TI **DRV5032DU** if you want an active hall (1.65–5.5 V, 1.8 µA, open-drain) | 1 | 10 s magnet hold → maintenance window (GPIO13). **Avoid A3144/KY-003/A1104:** 3.8 V min supply and 4–25 mA draw — wrecks the sleep budget | 20–60 |
...
1. **LoRa frequency must match** between collar (T-Beam variant) and base station module — buy the same band (433 MHz or 915 MHz) for both. Confirm which band the T-Beam variant ships with before ordering the base module. Radio variants: **SX1278 = 433 MHz** (order this for the PH 433 plan), SX1276 = 868/915 MHz, SX1262 = dual-band.
